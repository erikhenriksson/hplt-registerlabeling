...
class MemoryTracker:
    def __init__(self, rank):
        ...
        self.last_mem = self.get_memory()

    def get_memory(self):
        """Get current memory usage in MB"""
        return self.process.memory_info().rss / 1024 / 1024

    def log_memory(self, step, force_gc=False):
        """Log memory usage and difference from last check"""
        if force_gc:
            gc.collect()
            torch.cuda.empty_cache()

        current_mem = self.get_memory()
        gpu_mem = torch.cuda.memory_allocated(device=f"cuda:{self.rank}") / 1024 / 1024
        diff = current_mem - self.last_mem

        print(f"GPU {self.rank} - {step}:")
        print(f"  RAM: {current_mem:.1f}MB (Δ{diff:+.1f}MB)")
        print(f"  GPU: {gpu_mem:.1f}MB")

        self.last_mem = current_mem
        return current_mem, gpu_mem
...
def batch_process(
    ...
    memory_tracker = MemoryTracker(rank)
    ...
    while True:
        memory_tracker.log_memory(f"Start of batch {batch_count}")
        ...
        memory_tracker.log_memory(f"After loading batch {batch_count}")
        ...
        memory_tracker.log_memory(f"After tokenization batch {batch_count}")
        ...
        for i in range(0, len(sorted_indices), batch_size):
            ...
            if rank == 0 and processed_count % 1000 == 0:
                ...
                memory_tracker.log_memory(
                    f"After processing {processed_count} items", force_gc=True
                )

        processed_results.sort(key=lambda x: x[0])
        memory_tracker.log_memory(f"End of batch {batch_count}", force_gc=True)
        batch_count += 1
        yield processed_results

    print(f"GPU {rank}: Completed processing {processed_count} items")
    memory_tracker.log_memory("Final memory usage", force_gc=True)
