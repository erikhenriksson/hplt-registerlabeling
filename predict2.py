...
def monitor_memory(rank: int, threshold_gb: float = 32.0) -> Dict:
    ...
    print(
        f"GPU {rank}: RAM usage - {mem_info['rss']:.2f}GB, "
        f"VMS: {mem_info['vms']:.2f}GB, "
        f"FDs: {mem_info['fds']}, "
...
class DataIterator:
    """Iterator that reads data from a compressed file and distributes lines among processes."""
...
def batch_process(
    ...
    with local_data_adaptive(input_path, rank, world_size) as data_iterator:
...
class OrderedFileReader:
    ...
    def _read_next(self):
        try:
            ...
            print(f"Error reading line in file {self.file_path}: {e}")
            self.exhausted = True
            self.current_item = None
...
def merge_ordered_files(temp_files, output_path, buffer_size=8 * 1024 * 1024):
    ...
    with ExitStack() as stack:
        readers = [stack.enter_context(OrderedFileReader(f)) for f in temp_files]
        ...
        with open(output_path, "wb", buffering=buffer_size) as final_out:
            ...
            with cctx.stream_writer(final_out) as final_writer:
                ...
                while active_readers > 0:
                    ...
                    if min_pos != output_position:
                        print(
                            f"Position mismatch: expected {output_position}, got {min_pos}"
                        )
                        output_position = min_pos  # Adjusting to continue processing
...
def process_and_save_ddp(rank, cfg, world_size):
    ...
    try:
        ...
        try:
            ...
            print(f"Error during barrier synchronization on GPU {rank}: {e}")
